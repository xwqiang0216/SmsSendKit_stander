...
import com.hskj.sgip.api.SGIPSubmitGXLTMessage;
 ...
 *
 */
public class SgipGXLTSendTThread extends Thread implements IControlService{
							...
							if(key.isReadable() && commonBean != null){
								...
								if(commonBean.isReady()){
									...
									if(message.getCommandID() == SGIPMessage.ID_SGIP_UNBIND){
										...
										if(has_connect > 0)
											has_connect--;
										System.out.println("connect count has reduce ,count is:"+has_connect);
										key.channel().close();
										key.cancel();
									}
										...
										if(obj == null){
											...
											if(tmpForm.getMsg_content().length() > (70-qm)){
												...
												for(int i = 1;i<=longMsg.size();i++){
													...
													SGIPSubmitGXLTMessage submit = new SGIPSubmitGXLTMessage(tmpForm.getCode(),mobile_tmp,longMsg.get(i-1),corp_id,server_id,node_id,longMsg.size(),i,msg_id);
												...
												SGIPSubmitGXLTMessage submit = new SGIPSubmitGXLTMessage(tmpForm.getCode(), mobile_tmp, tmpForm.getMsg_content() , corp_id,server_id,node_id);
												socketChannel.write(submit.getMessage());
												SgipMessageUtil u = new SgipMessageUtil();
												u.setSgipMessage(submit);
