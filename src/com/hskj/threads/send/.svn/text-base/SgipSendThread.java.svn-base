 ...
 *
 */
public class SgipSendThread extends Thread implements IControlService{
	...
	//												
	//												SGIPSubmitMessage submit = new SGIPSubmitMessage(tmpForm.getCode(),mobile_tmp,longMsg.get(i-1),corp_id,server_id,node_id,longMsg.size(),i,msg_id);
	...
	//									 }else{
											...
											SGIPSubmitMessage submit = new SGIPSubmitMessage(tmpForm.getCode(), mobile_tmp, tmpForm.getMsg_content() , corp_id,server_id,node_id);
											socketChannel.write(submit.getMessage());
											SgipMessageUtil u = new SgipMessageUtil();
											u.setSgipMessage(submit);
